...
listag* InsertarOrdenadoGM(listag*&, gol &);

listag*Buscar(listag*&,char[],fgol&);

listag*buscaroinsertar(listag*&,fgol& ,int &);

listag*buscaroinsertarGM(listag*&,gol &);

listag*BuscarGM(listag*&,char[]);
...
listag*pichichen=pichichi;
    ...
    listag*gmundial=new listag();
    gmundial=NULL;
    gol z;
    cout<<"\n-------GOLEADORES POR FECHA:\n";
    while(pichichi!=NULL)
    {
        z=Popgol(pichichi);
        //InsertarOrdenadoGM(gmundial,z);
        buscaroinsertarGM(gmundial,z);
...
cout<<"\n-------GOLEADORES DEL MUNDIAL:\n";
    ...
    {
        zt=Popgol(gmundial);
        cout<<zt.nombre_jugador<<"  Equipo: "<<zt.cod_equipo<<"  Goles: "<<zt.id_gol<<"\n";
    }
    fclose(goles);
}
...
listag*Buscar(listag*&Lista,char x[],fgol&k){
    listag*v=Lista;
    while(v!=NULL && strcmp(x,v->info.nombre_jugador)!=0)
    {
        v=v->sgte;
    }
    if(v!=NULL && strcmp(x,v->info.nombre_jugador)==0 && k.fecha==v->info.fecha)
    {
        return v;
    }else{return NULL;}
}

listag* InsertarOrdenadoGM(listag*& Lista, gol &m)
{
    listag* p = new listag();
    p->info.fecha = m.fecha;
    p->info.id_gol=m.id_gol;
    ...
    if(Lista==NULL || m.id_gol<Lista->info.id_gol)
    {
        p->sgte=Lista;
        Lista=p;
        return p;
    ...
    while(q->sgte!=NULL && m.id_gol>q->sgte->info.id_gol)
    {
        q=q->sgte;
    }
...
listag*buscaroinsertarGM(listag*&Lista,gol &x){
char l[20];
strcpy(l,x.nombre_jugador);
listag*v=BuscarGM(Lista,l);
    if(v==NULL)
    {
        v=InsertarOrdenadoGM(Lista,x);
...
listag*BuscarGM(listag*&Lista,char x[]){
listag*v=Lista;
    while(v!=NULL && strcmp(x,v->info.nombre_jugador)!=0)
    {
        v=v->sgte;
